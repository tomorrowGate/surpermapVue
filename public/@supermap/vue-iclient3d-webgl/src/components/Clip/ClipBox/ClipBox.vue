<template>
  ...
  <div class="sm-function-module-content">
    <div class="sm-function-module-sub-section">
      ...
      <input class="min-solider" min="0" step="1" style="width:52%" type="range" v-model="clipLength"/>
      <input class="min-solider" min="0" step="1" style="width:22%; height:25px; margin-left:4%" type="number" v-model="clipLength"/>
    </div>
    <div class="sm-function-module-sub-section">
      <label class="label-container">宽度(米) :</label>
      <input class="min-solider" min="0" step="1" style="width:52%" type="range" v-model="clipWidth"/>
      <input class="min-solider" min="0" step="1" style="width:22%; height:25px; margin-left:4% " type="number" v-model="clipWidth"/>
    </div>
    <div class="sm-function-module-sub-section">
      <label class="label-container">高度(米) :</label>
      <input class="min-solider" min="0" step="1" style="width:52%" type="range" v-model="clipHeight"/>
      <input class="min-solider" min="0" step="1" style="width:22%; height:25px; margin-left:4%" type="number" v-model="clipHeight"/>
    </div>
    <div class="sm-function-module-sub-section">
      <label class="label-container">旋转(度) :</label>
      <input class="min-solider" max="180" min="-180" step="1" style="width:52%" type="range" v-model="clipRotate"/>
      <input class="min-solider" max="180" min="-180" step="1" style="width:22%; margin-left:4%; height:25px" type="number"
             v-model="clipRotate"/>
    </div>

    <div class="sm-function-module-sub-section">
      <label class="sm-function-module-sub-section-caption">裁剪模式:</label>
      <select class="sm-select" id="" style="width:79%" v-model="ClipModelSelected">
  ...
  <div class="sm-viewshed-panel" id="viewshed-panel" v-if="clipPlaneMode">
    <div
    ...
    <div :class="{'sm-viewshed-content-hidden' : !show}" class="sm-viewshed-content">
      ...
      <div class="sm-function-module-content">
        ...
        <div class="sm-function-module-sub-section">
              <label class="sm-function-module-sub-section-caption">裁剪模式:</label>
              <select class="sm-select" id="fillOptions" style="width:79%" v-model="ClipModelSelected">
                <option :value="Options.id" v-for="Options in ClipModels">{{Options.name}}</option>
              </select>
            </div>
            <div class="boxchild ">
  ...
  export default {
    ...
    watch:{
      clipLength(val) {
        ...
        this.boxEntity.box.dimensions = new Cesium.Cartesian3(newVal, dim.y, dim.z);
        this.setClipBox();
      },
      clipWidth(val) {
        if(!this.boxEntity) {
        ...
        let dim = this.boxEntity.box.dimensions.getValue();
        let newVal = Number(val)
        this.boxEntity.box.dimensions = new Cesium.Cartesian3(dim.x, newVal, dim.z);
        this.setClipBox();
      },
      clipHeight(val) {
        if(!this.boxEntity) {
        ...
        let dim = this.boxEntity.box.dimensions.getValue();
        let newVal = Number(val)
        this.boxEntity.box.dimensions = new Cesium.Cartesian3(dim.x, dim.y, newVal);
        this.setClipBox();
      },
      clipRotate(val) {
        if(!this.boxEntity) {
