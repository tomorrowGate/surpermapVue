<template>
  <div>
    <div v-if="measureComb">
      <div >
        <div class="sm-measure-btn-list">
          <button
            @click="distanceClk"
            class="sm-measure-btn"
            title="空间距离"
            type="button"
          >
          ...
          <button @click="areaClk" class="sm-measure-btn"  title="空间面积" type="button">
          ...
          <button @click="heightClk" class="sm-measure-btn"  title="高度" type="button">
          ...
          <button @click="clear" class="sm-measure-btn"  title="清除" type="button">
            <span class="iconfont iconVue-measure-clear"></span>
          </button>
        </div>
        <div class="sm-measure-half" v-show="isShowEllipsoid">
          <label class="sm-function-module-sub-section-caption" style>椭球:</label>
          <select class="sm-measure-select" v-model="EllipsoidMode">
        ...
        <div v-show="isShowDVH">
          <label class="third">
            <input checked  type="checkbox" v-model="isShowLine" />
            <span>显示等高线</span>
          </label>
        </div>
      </div>
  ...
  <div class="sm-measure-panel" id="measure-panel" v-if="dsMode" >
    <div
      :class="{'sm-measure-toggle-btn-only': !show}"
      @click="toggleVisibility"
      class="sm-measure-toggle-btn"
    >
      <span class="iconfont iconVue-measure"></span>
    </div>
    <div :class="{'sm-measure-content-hidden' : !show}" class="sm-measure-content">
      <div class="sm-measure-panel-header">
        <span>量算</span>
      </div>
      <div>
      <div class="sm-measure-btn-list">
        ...
        <button @click="areaClk" class="sm-measure-btn" id="area" title="空间面积" type="button">
          <span class="iconfont iconVue-measure-area"></span>
        </button>
        <button
          @click="GroundAreaClk"
        ...
        <button @click="heightClk" class="sm-measure-btn" id="height" title="高度" type="button">
          <span class="iconfont iconVue-measure-height"></span>
        </button>
        ...
        <button @click="clear" class="sm-measure-btn" id="clear" title="清除" type="button">
          <span class="iconfont iconVue-measure-clear"></span>
        </button>
      </div>
      <div class="sm-measure-half" v-show="isShowEllipsoid">
        <label class="sm-function-module-sub-section-caption" style>椭球:</label>
        <select class="sm-measure-select" id="EllipsoidMode" v-model="EllipsoidMode">
          <option selected value="圆球">圆球</option>
          <option selected value="CGCS2000">CGCS2000</option>
          <option value="XIAN80">XIAN80</option>
          <option value="WGS84">WGS84</option>
...
export default {
      ...
      if (window.viewer) {
        ...
        handlerDis.measureEvt.addEventListener(function(result) {
          if (that.clampMode === Cesium.ClampMode.Ground && that.Ellipsoid) {
            ...
            var dis = Number(
              scene.globe.computeSurfaceDistance(
                polylineGeometry,
                that.Ellipsoid
              )
            );
  ...
  beforeMount() {
    ...
    eventBus.$on("sendCname", (e) => {
      ...
      if (e == "量算") {
        this.measureComb = true;
      } else {
        this.measureComb = false
      }
    });
  }
};
